...
int main(int argc, char * argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("storing_groceries_node");
  ...
  factory.registerFromPlugin(loader.getOSName("determine_object_position"));
  ...
  factory.registerFromPlugin(loader.getOSName("detect_nearest_entity"));
  

  std::string pkgpath = ament_index_cpp::get_package_share_directory("robocup_behavior_trees");
  std::string xml_file = pkgpath + "/xml/storing_groceries.xml";
  ...
  std::cout << "Storing Groceries Finished with status: "<< status << std::endl;
  rclcpp::shutdown();
  return 0;
}